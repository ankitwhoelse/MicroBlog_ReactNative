...
class CasModeleUtilisateur(unittest.TestCase):
    ...
    def test_publications_suivies(self):
        ...
        p1 = Publication(corps="publication de patate", auteur=u1,
                    horodatage=maintenant + timedelta(seconds=1))
        p2 = Publication(corps="publication de tomate", auteur=u2,
                    horodatage=maintenant + timedelta(seconds=4))
        p3 = Publication(corps="publication de salade", auteur=u3,
                    horodatage=maintenant + timedelta(seconds=3))
        p4 = Publication(corps="publication de radis", auteur=u4,
                    horodatage=maintenant + timedelta(seconds=2))
        db.session.add_all([p1,p2,p3,p4])
        db.session.commit()

        #Mise en place des partisans
        u1.devenir_partisan(u2) # Patate est partisan de Tomate
        u1.devenir_partisan(u4) # Patate est partisan de Radis 
        u2.devenir_partisan(u3) # Tomate est partisan de Salade
        u3.devenir_partisan(u4) # Salade est partisan de Radis 
        db.session.commit()
